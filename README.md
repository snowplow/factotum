...
on the completion of the `echo alpha` task, and so will wait for `echo alpha` to complete. `echo omega` depends on the `echo beta` task, and so will wait for `echo beta` to be complete before executing.
...
Factotum is written in **[Rust](https://www.rust-lang.org/)** and requires **Rust 1.78.0**.

### Building and Testing

**Recommended: Using Docker (works on any platform)**
...
# Run tests on Ubuntu 18.04, 22.04, and 24.04
make test

# Or test specific Ubuntu versions
make test-docker-18    # Ubuntu 18.04
make test-docker-22    # Ubuntu 22.04
make test-docker-24    # Ubuntu 24.04
```

The Docker tests:
1. Run unit tests (without static linking - proc-macros incompatible with `crt-static`)
2. Build x86_64 binary with Rust 1.78.0 and static linking
3. Run functional tests of the statically-linked binary on all three target Ubuntu versions

**Note**: Both local and GitHub CI use Docker with `--platform=linux/amd64` to build x86_64 binaries with static linking (emulated on Mac, native on Linux). Unit tests run without static linking because proc-macro crates are incompatible with `crt-static`, but the final binary is statically linked and tested on all Ubuntu versions.
...
### Using Rust 1.78.0 directly

* **[Install Rust 1.78.0](https://www.rust-lang.org/tools/install)**
   ```bash
...
**Target Platforms:**
- Ubuntu 18.04 (OpenSSL 1.1.1, glibc 2.27)
- Ubuntu 22.04 (OpenSSL 3.0, glibc 2.35)
- Ubuntu 24.04 (OpenSSL 3.0, glibc 2.39)

For more details, see **[CLAUDE.md](CLAUDE.md)**. 

## Copyright and license

Factotum is copyright 2016-current Snowplow Analytics Ltd.
