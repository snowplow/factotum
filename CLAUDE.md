...
- **Linux x86_64**: Uses Docker with `make test-docker-build` (same as CI) for static linking
- **Linux arm64**: Uses Docker with QEMU emulation and `--platform=linux/arm64` for static linking
- **macOS builds**: Direct cargo builds without static linking (not needed on macOS)
- Static linking ensures Linux binaries work across Ubuntu 18.04, 22.04, and 24.04

**Build Configuration:**

- **CI**: Uses `make test` which runs Docker-based build and test workflow
- **CD Linux builds**: Use Docker with static linking to ensure cross-Ubuntu compatibility
- **CD macOS builds**: Use direct cargo builds (no static linking needed)
- Docker builds use `--platform=linux/amd64` (x86_64) or `--platform=linux/arm64` (ARM64) to enable `crt-static`
- Static linking (`RUSTFLAGS="-C target-feature=+crt-static"`) ensures Linux binaries work across Ubuntu 18.04 (glibc 2.27, OpenSSL 1.1.1), 22.04 (glibc 2.35, OpenSSL 3.0), and 24.04 (glibc 2.39, OpenSSL 3.0)
- Unit tests always run WITHOUT `crt-static` due to proc-macro incompatibility
- Release binaries are built WITH `crt-static` for cross-version compatibility
